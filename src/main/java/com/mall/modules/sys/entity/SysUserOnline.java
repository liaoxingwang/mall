...
import com.mall.common.utils.AddressUtils;
...
public class SysUserOnline extends AbstractBaseEntity<SysUserOnline> {
	...
	private Date startTimestsamp;		// session创建时间
	private Date lastAccessTime;		// session最后访问时间
	private Integer expireTime;		// 超时时间，单位为分钟
	private Date beginStartTimestsamp;		// 开始 session创建时间
	private Date endStartTimestsamp;		// 结束 session创建时间
	private Date beginLastAccessTime;		// 开始 session最后访问时间
	private Date endLastAccessTime;		// 结束 session最后访问时间
